...
func InitLeds(r renderer.Renderer, freq int, ledCount1 int, ledCount2 int) Leds {
...
func (l Leds) SetLedRed(channel int, led int) {
	fmt.Printf("l.Clear()\n")
	l.Clear()

	fmt.Printf("l.SetLed(%+v)\n", led)
	l.SetLed(channel, led, Red)
...
// Turn off all LEDs
func (l Leds) ClearAll(ledCount1 int, ledCount2 int) {
	fmt.Printf("l.Init()\n")
	err := l.Init(
		DEFAULT_FREQ,
		GPIO_PIN1A, ledCount1, BRIGHTNESS,
